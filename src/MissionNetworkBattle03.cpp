...
int MissionNetworkBattle03::processtimer(Uint32 dt) {
    ...
    if (MissionNetworkBattle03Timer > 1000) { //  Get here once every second.
    ...
    if (!MissionRunning) { //  Get here if this is the FIRST pass through processtimer().
        ...
        if (fplayer->id == FIGHTER_ME163) { //  Special case: The Me163 only gets 6% fuel in this mission.
    ...
    if (MissionStateNetworkBattle == 2 && (timer > PriorStateEndTimer + 1100)) {
        ...
        if (MyNetworkId % 2) { //  RedTeam
            if (ThreeDObjects[MissionObjectHqRed]->Durability < ThreeDObjects[MissionObjectHqRed]->maxDurability * 0.4) { //  Get here if player's RedTeam HQ is heavily damaged. Diminish fuel/armament for late-war aircraft.
                ...
                if (fplayer->HistoricPeriod > 1) { //  Get here if the player is flying a late-war or postwar aircraft in this MissionNetworkBattle03
    ...
    for (MissionAircraftNumber = 1; MissionAircraftNumber <= 10; MissionAircraftNumber++) {
        ...
        if (!ThreeDObjects[MissionAircraftNumber]->active && (myrandom(10000) > 9990)) { //  Get here 0.5% of the time when any other plane explodes or crashes. Re-spawn destroyed aircraft.
...
// Check to see if the player's aircraft has been destroyed:
    ...
    if (NetworkReceiveTimer > NetworkReceiveTimerInterval) {
        ...
        if (BytesReceived == sizeof(LacUdpApiPacket)) { //  Get here if a new InPacket has arrived.
            ...
            if (MissionNetworkBattle03PriorAircraft[InPacket.UdpObjPlayerNumber] != InPacket.UdpObjVehicle) { //  Get here if this network peer is in a different aircraft
                //
