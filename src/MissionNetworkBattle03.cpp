MissionNetworkBattle03::MissionNetworkBattle03 ()
    ...
    //
    display ((char *)"@MissionNetworkBattle03::MissionNetworkBattle03()", LOG_MOST);
    ...
    if ((NetworkMode & 1) == 1)
      ...
      display ((char *)"@Attempting MissionNetworkBattle03 in ClientServer Network mode.", LOG_NET);
      if (OpenClientUdpSocket() == 0)
        { //  Get here if we were able to open the required ClientServer UDP Socket
        display ((char *)"@MissionNetworkBattle03 ClientServer mode succesfully opened ClientUDP socket", LOG_NET);
        ConfigureClientUdpSocket();
        }
      else
        { //  Get here if we were unable to open the required ClientUDP socket
        display ((char *)"@MissionNetworkBattle03 ClientServer mode unable to open ClientUDP socket.", LOG_NET);
        }
      }
    else
      {
    ...
    //
    event_CommsMacro17();
    } //  end MissionNetworkBattle03::MissionNetworkBattle03 ()
...
void MissionNetworkBattle03::start ()
    ...
    display ((char *)"@MissionNetworkBattle03::start()", LOG_MOST);
    ...
    SeaLevel = -12.415; //  Must be adjusted for some terrains.
    //
    ...
    //
    int n = MissionObjectBattleshipRed1; //  RedTeam Battleship
    ...
    n = MissionObjectRadarRed1; //  RedTeam Rectangular RADAR Reflector
    ThreeDObjects [n]->tl->x = AirfieldXMin + 41.0;
    ...
    n = MissionObjectBunkerRed1; //  Just a bunker building on top of an adjacent hill.
    ...
    n = MissionObjectHqRed; //  RedTeam HQ airfield facilities
    ThreeDObjects [n]->tl->x = AirfieldXMin + 46.5;
    ...
    //  Now insert "Airfield00" structures onto the landscape near the runway, treating those structures as if
    //  they were a single fighter aircraft so they can be damaged and/or destroyed. This will be the HQ
    //  airfield for the RedTeam.
    //

    n = MissionObjectBattleshipBlue1; //  BlueTeam Battleship
    ...
    n = MissionObjectRadarBlue1; //  BlueTeam Rectangular RADAR Reflector
    ...
    n =  MissionObjectBunkerBlue1; //  Just a bunker building on top of an adjacent hill.
...
#ifndef STEAMDECK
    event_RadarOnOff();         //  Turn on RADAR for this mission unless on Steam Deck
#endif
    ...
    } //  end MissionNetworkBattle03::start ()
...
int MissionNetworkBattle03::processtimer (Uint32 dt)
    ...
    MissionNetworkBattle03Timer += DeltaTime; //  Advance by milliseconds
    MissionNetworkBattleRadarTimer += DeltaTime; //  Advance by milliseconds
    MumbleTimeRemainingBeforeChannelReset -= DeltaTime;

//sprintf (DebugBuf, "MissionNetworkBattle03::processtimer() Function entry: MissionNetworkBattle03Timer = %d. fplayer->Durability = %f.", MissionNetworkBattle03Timer, fplayer->Durability);
//display (DebugBuf, LOG_MOST);
    ...
    if (MissionNetworkBattleRadarTimer > 100)
       { //  Get here 10 times /second. Rotate the two RADAR antennas.
       MissionNetworkBattleRadarTimer = 0;
       if (ThreeDObjects[MissionObjectRadarBlue1]->Durability > 200)
       ...
       if (ThreeDObjects[MissionObjectRadarRed1]->Durability > 200)
    ...
    if (MissionNetworkBattle03Timer > 1000)
       ...
       FratricideShameFactor *= 0.85; //  Shame doesn't last forever unless it is extreme....
       if (FratricideShameFactor < 10)
          {
          FratricideShameFactor = 0;
          }
       if (FratricideShameFactor > 100)
          { //  Get here if we should be ashamed of ourselves for fratricide
          ; //  Don't punish for fratricide in this MissionNetworkBattle03.
          }
       int AircraftCount;
       ...
       } //  end of (MissionNetworkBattle03Timer > 1000)
    ...
    if (StaticObjectUpdateTimer > 4000)
       ...
       if (TeamSwitcher == false)
          { //  Get here if it's time to handle Blue Team facilities
          TeamSwitcher = true;
          if ((ThreeDObjects[MissionObjectHqBlue]->Durability > 0) && ((ThreeDObjects[MissionObjectHqBlue]->Durability) < (ThreeDObjects[MissionObjectHqBlue]->maxDurability)))
             { //  Get here if BlueTeam HQ Airfield facilities have been damaged
          ...
          { //  Get here if it's time to handle Red Team facilities
          TeamSwitcher = false;
          if ((ThreeDObjects[MissionObjectHqRed]->Durability > 0) && ((ThreeDObjects[MissionObjectHqRed]->Durability) < (ThreeDObjects[MissionObjectHqRed]->maxDurability)))
             { //  Get here if RedTeam HQ Airfield facilities have been damaged
    ...
    if (!MissionRunning)
       ...
       fplayer->FuelLevel = 96.0; //  Aircraft do NOT need to land for fuel at start of this mission.
       if (fplayer->id == FIGHTER_ME163)
          { //  Special case: The Me163 only gets 6% fuel in this mission.
       ...
       */
       ...
       //  BlueTeam Battleship altitude
       ThreeDObjects[MissionObjectBattleshipBlue1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectBattleshipBlue1]->tl->y += 0.5;

       //  RedTeam Battleship altitude
       ThreeDObjects[MissionObjectBattleshipRed1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectBattleshipRed1]->tl->y += 0.5;

       //  Airfield 0 facilities altitude
       ThreeDObjects[MissionObjectRadarBlue1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectRadarBlue1]->tl->y += 5.4;
       ThreeDObjects[MissionObjectBunkerBlue1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectBunkerBlue1]->tl->y += 5.6;
       ThreeDObjects[MissionObjectHqBlue]->tl->y = l->getHeight(AirfieldXMin+5, AirfieldYMin+5);
       ThreeDObjects[MissionObjectHqBlue]->tl->y +=0.90;
       //  Airfield 1 facilities altitude
       ThreeDObjects[MissionObjectRadarRed1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectRadarRed1]->tl->y += 5.4;
       ThreeDObjects[MissionObjectBunkerRed1]->tl->y = l->getExactHeight(-467, 28);
       ThreeDObjects[MissionObjectBunkerRed1]->tl->y += 5.4;
       ThreeDObjects[MissionObjectHqRed]->tl->y = l->getHeight(AirfieldXMin+5, AirfieldYMin+5);
       ThreeDObjects[MissionObjectHqRed]->tl->y +=0.90;
       ...
       if (fplayer->missiles [0] >= 4 || fplayer->id == FIGHTER_D3A || fplayer->id == BOMBER_B5N)
          ...
          if (AirfieldRequested >= 3)
             ...
             fplayer->tl->y += 320; // This is a bomber airfield. Give them extra altitude at start. 320 yields 9400 ft. 650 yields 17000ft.
    ...
    if (timer > 10000 && MissionStateNetworkBattle == 0 && NetworkTransmitTimer >= NetworkTransmitTimerInterval)
       { //  Mission has just begun and we've just begun transmitting OutPackets with an available setting for MyNetworkId
       if (MyNetworkId % 2)
       ...
       if (MyNetworkId%2)
          ...
          if (ThreeDObjects[MissionObjectHqRed]->Durability < ThreeDObjects[MissionObjectHqRed]->maxDurability * 0.4)
             ...
             if (fplayer->HistoricPeriod > 1)
                { //  Get here if the player is flying a late-war or postwar aircraft in this MissionNetworkBattle03
                ...
                for (i2 = 0; i2 < missiletypes; i2 ++)
                    ...
                    if (i2 == 0)
                       ...
                       display ((char *)"MissionNetworkBattle03::processtimer() diminishing RedTeam late-war bomb load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    if (i2 == 5)
                       { //  Rack5 = DFM Rockets
                       fplayer->missiles [i2] /=3;
                       display ((char *)"MissionNetworkBattle03::processtimer() diminishing RedTeam late-war rocket load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    }
                }
             else
             ...
             if (fplayer->HistoricPeriod > 1)
                ...
                for (i2 = 0; i2 < missiletypes; i2 ++)
                    ...
                    if (i2 == 0)
                       { //  Rack0 = BOMB01
                       fplayer->missiles [i2] /= 3;
                       display ((char *)"MissionNetworkBattle03::processtimer() diminishing BlueTeam late-war bomb load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    if (i2 == 5)
                       { //  Rack5 = DFM Rockets
                       fplayer->missiles [i2] /=3;
                       display ((char *)"MissionNetworkBattle03::processtimer() diminishing BlueTeam late-war rocket load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    }
                }
             else
    ...
    if (MissionStateNetworkBattle == 10 && (timer > PriorStateEndTimer +1500))
       {
       sound->setVolume (SOUND_MISSIONNETWORKBATTLE03, 126);
    ...
    for (MissionAircraftNumber=1; MissionAircraftNumber <= 10; MissionAircraftNumber ++)
        ...
        if (ThreeDObjects [0]->explode > 500 * timestep)
           { //  Player's aircraft exploded recently but has not been re-spawned and player hasn't pressed <ESC>.
        ...
        if (!ThreeDObjects[MissionAircraftNumber]->active && (myrandom(10000)>9990))
            { //  Get here 0.5% of the time when any other plane explodes or crashes. Re-spawn destroyed aircraft.
            ...
            if (ThreeDObjects[MissionAircraftNumber]->Sentient > 3)
               {//  Get here if the destroyed aircraft was piloted by a Sentient
               ...
               MissionNetworkBattle03PriorAircraft[MissionAircraftNumber] = ThreeDObjects[MissionAircraftNumber]->id;
               }
            ...
            ThreeDObjects [MissionAircraftNumber]->MyMissionPlayerNumber = MissionAircraftNumber; //  Make this object's "MyMissionPlayerNumber" globally visible
            //
            ...
            ThreeDObjects[MissionAircraftNumber]->tl->y += 100 + (myrandom(80) - 40);
            ThreeDObjects [MissionAircraftNumber]->Sentient = 0;
            ThreeDObjects [MissionAircraftNumber]->thrustUp();
            ThreeDObjects [MissionAircraftNumber]->thrustUp();
            ThreeDObjects [MissionAircraftNumber]->manoeverstate = 0;
            sprintf (
                    DebugBuf,
    ...
    if (NetworkReceiveTimer > NetworkReceiveTimerInterval )
       ...
       if (BytesReceived == sizeof (LacUdpApiPacket))
          ...
          if (MissionNetworkBattle03PriorAircraft[InPacket.UdpObjPlayerNumber] != InPacket.UdpObjVehicle)
             { //  Get here if this network peer is in a different aircraft
              //
    ...
    if (MissionEndingTimer)
       { //  Get here if one of the two teams has won the battle
       if (MissionEndingTimer == 13000 && !MissionEndingTimer2)
          { // Get here just once, as soon as an airfield is destroyed.
          display ((char *)"MissionNetworkBattle03::processtimer(): Playing SOUND_MISSIONENDINGINSTRUMENTAL.", LOG_MOST);
          if ((ThreeDObjects[MissionObjectHqBlue]->Durability) < ((ThreeDObjects[MissionObjectHqBlue]->maxDurability) * 0.33))
             { // Get here when BlueHQ is destroyed.
             ...
             NewSystemMessageNeedsScrolling = true;
             if (MyNetworkId & 1)
                { //@ Get here if player is on RedTeam. Celebrate victory:
                sound->play (SOUND_MISSIONENDINGINSTRUMENTAL, false); // Joyful trumpet fanfare
                }
             else
                { //@ Get here if player is on BlueTeam. No celebration.
                sound->play (SOUND_DEFEAT00, false); // Sad mood music
                }
             }
          else if ((ThreeDObjects[MissionObjectHqRed]->Durability) < ((ThreeDObjects[MissionObjectHqBlue]->maxDurability) * 0.33))
             { // Get here when RedHQ is destroyed.
             ...
             NewSystemMessageNeedsScrolling = true;
             if (MyNetworkId & 1)
                { //@ Get here if player is on RedTeam. No celebration.:
                sound->play (SOUND_DEFEAT00, false); // Sad mood music
                }
             else
                { //@ Get here if player is on BlueTeam. Celebrate victory.
                sound->play (SOUND_MISSIONENDINGINSTRUMENTAL, false); // Joyful trumpet fanfare
                }
             }
          NewSystemMessageNeedsScrolling = true;
    ...
    if (MissionEndingTimer2)
       ...
       if (MissionEndingTimer2 < 1000)
          { //  Get here when all conditions to forcibly end mission are satisfied
          display ((char *)"MissionNetworkBattle03::processtimer(): Mission ending now.", LOG_MOST);
          fplayer->Durability = -1.0;
          if ((ThreeDObjects[MissionObjectHqBlue]->Durability) < ((ThreeDObjects[MissionObjectHqBlue]->maxDurability) * 0.33))
             { // Get here if RedTeam won the battle.
             ...
             { // Get here if BlueTeam won the battle.
             if (MissionEndingTimer2 > 800)
                {
    ...
    return 0;
    } //  end MissionNetworkBattle03::processtimer (Uint32 dt)
...
void MissionNetworkBattle03::draw ()
    ...
    } //  end MissionNetworkBattle03::draw ()
...
DamageToClaim = 0.0;
return (false);
} //  end MissionNetworkBattle03RetrieveFirstDamageDescription()
