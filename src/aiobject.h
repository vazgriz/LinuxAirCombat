...
#define STATIC_CARRIER00 10202
#define STATIC_DESTROYER 10203
#define STATIC_BATTERY 10204
...
class DynamicObj : public CSpaceObj
{
  ...
  float recgamma; // climb angle the fighter wants to reach
  ...
  float ruddereffect;
  float rolleffect;
  float recelevatoreffect; // Recommended elevator effect number between 1.0 and -0.5, as diving should be less
  float recruddereffect; // Recommended rudder effect.
  float recrolleffect; // Recommended roll effect
  ...
  float FuelBurnRate = 1.0; // When NOT 1.0, some aircraft consume their fuel supply faster or slower than others.
  float TorqueMultiplier = 1.0; // Aircraft whose engines/configurations do not generate torque set this to 0.0
...
class AIObj : public DynamicObj
{
  ...
  float AileronFromPriorInPacket = 0.0;
  float ElevatorFromPriorInPacket = 0.0;
  float RudderFromPriorInPacket = 0.0;
  float ThrustFromPriorInPacket = 0.0;
  int flares;
  int chaffs;
  int fireflarettl;
  int firechaffttl;
  ...
  int CombatMode; // Current TacticalManoever is leading to what?
  int TacticalManoever; // current manoeverstate is leading to what tactic?
  int manoeverstate; // changes to realistic manoevers.
  int PriorManoeverState; // The most recently completed manoeverstate
  int ManoeverStateTimer; // Measures time in each manoeverstate
  int NextManoeverStateDuration; // Length limit for next manoeverstate
  int ManoeverStateTargetAltitude; // How high should this manoeverstate attempt to fly?
