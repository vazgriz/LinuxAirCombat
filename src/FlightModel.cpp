...
if (difficulty <= 2)
   { // Get here if ignoring Torque roll
   aileron *= fabs (aileron);
   }
else
   { // Get here if the player has configured us to consider Torque roll
   aileron *= fabs (aileron); 
...
//
if (NetworkMode & 32)
   { // Get here if our throttle level is configured for "relative mode"
   if (throttleinput == PriorThrottleReference)
      { // Get here if, due to some mysterious quirk, throttleinput gets corrupted by PriorThrottleReference.
      throttleinput = 0; // Discard this quirky little problem.
      }
   if ((throttleinput > -4000) && (throttleinput < 4000))
      { // Get here when throttle is at or near spring-loaded center position. Ignore it.
      ThrottleTemp = PriorThrottleReference;
      } // End logic for centered joystick
    else
      { // Get here if throttle is NOT centered.
      if (throttleinput < -4000)
         { // Get here when throttle lever is higher than center.
         ThrottleTemp -= 300;
         if (ThrottleTemp < -32000)
            { // Prevent it from asserting too much throttle
            ThrottleTemp = -32000;
            }
         PriorThrottleReference = ThrottleTemp;
         }
      if (throttleinput > 4000)
         { // Get here when throttle lever is lower than center.
         ThrottleTemp += 300;
         if (ThrottleTemp > 32000)
            { // Prevent it from diminishing throttle too far.
            ThrottleTemp = 32000;
            }
         PriorThrottleReference = ThrottleTemp;
         }
