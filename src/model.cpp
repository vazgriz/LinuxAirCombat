...
void CModel::draw(CVector3* tl, CVector3* tl2, CRotation* rot, float zoom, float lum, int explode) {
    ...
    float distance = 90.0 + 20.0 * (float)quality;
    if (MissionActive && ((abs(tl2->x - camx) > distance) || (abs(tl2->y - camy) > distance) || (abs(tl2->z - camz) > distance))) {
        if (!SelectingAircraft) {
            return;
        }
    }
