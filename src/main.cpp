...
char PathToActiveExecutable[ PATH_MAX ] = "Unknown";
...
int main (int argc, char **argv)
    ...
    sprintf (DebugBuf, "PathToActiveExecutable[] defaults to %s", PathToActiveExecutable);
    display (DebugBuf, LOG_MOST);
    GetExePath();
    sprintf (DebugBuf, "Actual path to Active Executable is %s", PathToActiveExecutable);
    ...
    sprintf (DebugBuf, "main(): PathToActiveExecutable = %s", PathToActiveExecutable);
    display (DebugBuf, LOG_MOST);

    sprintf (FileSystemDefaultHeightMapFilePath, "%s", PathToActiveExecutable);
   ...
   if (NetworkMode & 4)
      { // LAC is configured to ignore Mumble management
      MumbleManagement = false;
      }
   else
      { // LAC is configured to automatically manage Mumble
...
void createMenu ()
    ...
    */
    ...
    if (strcmp(CommunityHandle, "00000000000000") == 0 )
       { // Get here if user has not configured his CommunityHandle.
...
void event_thrustDown ()
    ...
    if (!MouseControlFlag)
       ...
       if (LandedAtSafeSpeed)
          { // Get here after landing while using keys for throttle
          ...
          { // Using keyboard throttle, but still in flight
          if (ThrottleReference > -20)
             {
             ThrottleReference = -20;
             }
...
void event_ToggleViewIntExt()
    ...
    if (camera==0)
        { // Get here upon entry into external view mode.
        camera = 1;
...
void GetExePath()
   {
   display ((char *)"GetExePath() function entry.", LOG_MOST);
   char * PointerToLastSlash;
   int    Slash = '/';
   int Index = 0;
   readlink( "/proc/self/exe", PathToActiveExecutable, PATH_MAX );

   PointerToLastSlash = strrchr(PathToActiveExecutable, Slash);

   Index = PointerToLastSlash - PathToActiveExecutable;
   Index++;
   PathToActiveExecutable[Index] = 0;
   }
...
{
if (Realm%2)
   { // Get here when Realm is an odd number.
   ...
   { // Get here when Realm is an even number.
       ...
       if (TapMenuTimer > 3000)
          ...
          switch (TapMenuCounter)
              ...
              case 8:
                ...
                TapMenuCounter = 0;
                if (MyNetworkId & 1)
                   { // Get here if player is on RedTeam
              ...
              case 9:
                {
                if (!(NetworkMode & 4))
                   { // Get here if mumble is installed and we are configured to manage it.
