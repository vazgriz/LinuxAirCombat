...
int MissionNetworkBattle01::processtimer(Uint32 dt) {
    ...
    if (MissionNetworkBattle01Timer > 1000) { //  Get here once every second.
    ...
    if (MissionStateNetworkBattle == 2 && (timer > PriorStateEndTimer + 1100)) {
        ...
        //  is asserted:
        //
        if (MyNetworkId % 2) { //  RedTeam MissionNetworkBattle01
            if (ThreeDObjects[MissionObjectHqRed]->Durability < ThreeDObjects[MissionObjectHqRed]->maxDurability * 0.4) { //  Get here if player's RedTeam HQ is heavily damaged. Diminish fuel/armament for late-war aircraft.
                ...
                if (fplayer->HistoricPeriod > 1) { //  Get here if the player is flying a late-war or postwar aircraft in this MissionNetworkBattle01
        ...
        } else { //  BlueTeam MissionNetworkBattle01
...
// Check to see if the player's aircraft has been destroyed:
    ...
    if (NetworkReceiveTimer > NetworkReceiveTimerInterval) {
        ...
        if (BytesReceived == sizeof(LacUdpApiPacket)) { //  Get here if a new InPacket has arrived.
            ...
            if (MissionNetworkBattle01PriorAircraft[InPacket.UdpObjPlayerNumber] != InPacket.UdpObjVehicle) { //  Get here if this network peer is in a different aircraft
                //
