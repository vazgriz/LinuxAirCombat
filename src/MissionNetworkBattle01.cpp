...
MissionNetworkBattle01::MissionNetworkBattle01 ()
    ...
    //
    display ((char *)"@MissionNetworkBattle01::MissionNetworkBattle01()", LOG_MOST);
    ...
    if ((NetworkMode & 1) == 1)
      ...
      display ((char *)"@Attempting MissionNetworkBattle01 in ClientServer Network mode.", LOG_NET);
      if (OpenClientUdpSocket() == 0)
        { //  Get here if we were able to open the required ClientServer UDP Socket
        display ((char *)"@MissionNetworkBattle01 ClientServer mode succesfully opened ClientUDP socket", LOG_NET);
        ConfigureClientUdpSocket();
        }
      else
        { //  Get here if we were unable to open the required ClientUDP socket
        display ((char *)"@MissionNetworkBattle01 ClientServer mode unable to open ClientUDP socket.", LOG_NET);
        }
      }
    else
      {
    ...
    //
    event_CommsMacro17();
    } //  end MissionNetworkBattle01::MissionNetworkBattle01 ()
...
void MissionNetworkBattle01::start ()
    ...
    display ((char *)"@MissionNetworkBattle01::start()", LOG_MOST);
    ...
    } //  end MissionNetworkBattle01::start ()
...
int MissionNetworkBattle01::processtimer (Uint32 dt)
    ...
    MissionNetworkBattle01Timer += DeltaTime; //  Advance by milliseconds
    ...
    if (MissionNetworkBattle01Timer > 1000)
       ...
       } //  end of (MissionNetworkBattle01Timer > 1000)
    ...
    if (MissionStateNetworkBattle == 2 && (timer > PriorStateEndTimer +1100))
       ...
       //
       if (MyNetworkId%2)
          { //  RedTeam MissionNetworkBattle01
          if (ThreeDObjects[MissionObjectHqRed]->Durability < ThreeDObjects[MissionObjectHqRed]->maxDurability * 0.4)
             ...
             if (fplayer->HistoricPeriod > 1)
                { //  Get here if the player is flying a late-war or postwar aircraft in this MissionNetworkBattle01
                ...
                for (i2 = 0; i2 < missiletypes; i2 ++)
                    ...
                    if (i2 == 0)
                       ...
                       display ((char *)"MissionNetworkBattle01::processtimer() diminishing RedTeam late-war bomb load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    if (i2 == 5)
                       { //  Rack5 = DFM Rockets
                       fplayer->missiles [i2] /=3;
                       display ((char *)"MissionNetworkBattle01::processtimer() diminishing RedTeam late-war rocket load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    }
                }
             else
             ...
             display ((char *)"MissionNetworkBattle01 state 1: Player's RedTeam HQ is NOT heavily damaged.", LOG_MOST);
             sprintf (SystemMessageBufferA, "OUR HQ FACILITIES PERMIT FULL FUEL/ORDNANCE.");
             NewSystemMessageNeedsScrolling = true;
             }
          }
       else
          { //  BlueTeam MissionNetworkBattle01
             ...
             if (fplayer->HistoricPeriod > 1)
                ...
                for (i2 = 0; i2 < missiletypes; i2 ++)
                    ...
                    if (i2 == 0)
                       { //  Rack0 = BOMB01
                       fplayer->missiles [i2] /= 3;
                       display ((char *)"MissionNetworkBattle01::processtimer() diminishing BlueTeam late-war bomb load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    if (i2 == 5)
                       { //  Rack5 = DFM Rockets
                       fplayer->missiles [i2] /=3;
                       display ((char *)"MissionNetworkBattle01::processtimer() diminishing BlueTeam late-war rocket load due to heavy HQ airfield damage", LOG_MOST);
                       }
                    }
                }
             else
    ...
    for (MissionAircraftNumber=1; MissionAircraftNumber <= 10; MissionAircraftNumber ++)
        ...
        if (!ThreeDObjects[MissionAircraftNumber]->active && (myrandom(10000)>9990))
            ...
            if (ThreeDObjects[MissionAircraftNumber]->Sentient > 3)
               ...
               MissionNetworkBattle01PriorAircraft[MissionAircraftNumber] = ThreeDObjects[MissionAircraftNumber]->id;
               }
    ...
    if (MissionEndingTimer)
       ...
       if (MissionEndingTimer == 13000 && !MissionEndingTimer2)
          ...
          display ((char *)"MissionNetworkBattle01::processtimer(): Playing SOUND_MISSIONENDINGINSTRUMENTAL.", LOG_MOST);
    ...
    return 0;
    } //  end MissionNetworkBattle01::processtimer (Uint32 dt)
...
void MissionNetworkBattle01::draw ()
    ...
    } //  end MissionNetworkBattle01::draw ()
...
DamageToClaim = 0.0;
return (false);
} //  end MissionNetworkBattle01RetrieveFirstDamageDescription()
