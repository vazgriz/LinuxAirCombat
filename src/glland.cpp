...
// Get terrain type over landscape/water, no interpolation (fast)
char GLLandscape::getTerrainType(float x, float z) {
    int mx = GETCOORD((int)floor(x));
    int mz = GETCOORD((int)floor(z));
    return (tex1[mx][mz]);
}
...
GLLandscape::GLLandscape(Space* space2, int type, int* heightmask) {
    ...
    //
    // Associate RGB colors with textures for every defined type of material.
    // If you want to make a runway or landscape area look different, you
    // can adjust the RGB brightness or the associated texture here.
    //
    for (i = 0; i < MAXMATERIAL; i++) {
        if (i == GRASS) {
            ...
            //setMaterial (i, 0.2, 0.2, 0.7, texwater);
            setMaterial(i, 0.0, 1.0, 0.0, texwater);
        } else if (i == SHALLOWWATER) {
            // This actually shows as sand of the beaches. Dunno why...
            setMaterial(i, 1.0, 1.0, 1.0, texwater); // White sand
        } else if (i == DEEPWATER) { // Doesn't seem to get used anywhere.
            setMaterial(i, 0.0, 0.0, 1.0, texwater);
        } else if (i == ROCKS2) {
            setMaterial(i, 0.5, 0.5, 0.5, texrocks);
        } else if (i == XSHALLOWWATER) {
            // This also shows up as sand of the beaches. Dunno why.
            setMaterial(i, 0.6, 0.6, 0.6, texwater); // Grey sand
        } else if (i == XDEEPWATER) { // Doesn't seem to get used anywhere.
            setMaterial(i, 0.0, 0.0, 0.7, texwater);
        } else if (i == REDSTONE) {
            ...
            setMaterial(i, 0.01, 0.01, 0.01, texrocks);
        } else {
            setMaterial(i, 0.4, 0.8, 0.3, texgrass);
        }
    }
