    ...
    if (ActiveTerrainFile != NULL)
       ...
       for (ArrayIndexY=0; ArrayIndexY < MAXX; ArrayIndexY++)
          ...
          for (ArrayIndexX=0; ArrayIndexX<MAXX; ArrayIndexX++)
            ...
            if (MissionNumber == 17)
               { 
               h[ArrayIndexX][ArrayIndexY] = ((LineBuffer[ArrayIndexX]) * 0.22) + 38000; 
               }
            else
               {
...
void Landscape::genDesertSurface (int hoehepc)
    ...
    int ArrayIndexY=0;
    unsigned short int LineBuffer [MAXX +1];
    display ((char *)"genDesertSurface(): Function entry.", LOG_MOST);
    ...
    if (ActiveTerrainFile != NULL)
       ...
       for (ArrayIndexY=0; ArrayIndexY < MAXX; ArrayIndexY++)
          ...
          for (ArrayIndexX=0; ArrayIndexX<MAXX; ArrayIndexX++)
            ...
            if (MissionNumber == 17)
               { 
               h[ArrayIndexX][ArrayIndexY] = LineBuffer[ArrayIndexX]*0.50; 
               }
            else
               {
...
void Landscape::calcWoods (int dy)
    ...
    for (AirfieldX = AirfieldXMin; AirfieldX < AirfieldXMax; AirfieldX++)
       ...
       for (AirfieldY = AirfieldYMin; AirfieldY < AirfieldYMax; AirfieldY++)
          { 
          f [AirfieldX] [AirfieldY] = RUNWAY; // Specify the color/texture of runways
          }
       }
    } 
