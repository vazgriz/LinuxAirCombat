...
//  terrain style and to experiment with various weather effects, or to ////popen
...
//  battleships, etc.
//
//  Developers are essentially un////popened in the design of missions deriving
//  from this framework.
//
...
int MissionNetworkBattle07::processtimer(Uint32 dt) {
 ...
 //
    ...
    if (MissionStateNetworkBattle == 2 && (timer > PriorStateEndTimer + 1100)) { //  Only get here once, 1100 sec after MissionStateNetworkBattle
        ...
        //
        if (MyNetworkId % 2) { //  RedTeam MissionNetworkBattle07
            if (ThreeDObjects[MissionObjectHqRed]->Durability < ThreeDObjects[MissionObjectHqRed]->maxDurability * 0.4) { //  Get here if player's RedTeam HQ is heavily damaged. Diminish fuel/armament for late-war aircraft.
                ...
                if (fplayer->HistoricPeriod > 1) { //  Get here if the player is flying a late-war or postwar aircraft in this MissionNetworkBattle07
        ...
        } else { //  BlueTeam MissionNetworkBattle07
...
// Check to see if the player's aircraft has been destroyed:
    ...
    if (MissionNetworkBattle07Timer > 1000) { //  Get here once every second.
    ...
    if (NetworkReceiveTimer > NetworkReceiveTimerInterval) { //  Get here every time NetworkReceiveTimer expires. Check for new InPacket:
        ...
        if (BytesReceived == sizeof(LacUdpApiPacket)) { //  Get here if a new InPacket has arrived.
            ...
            if (MissionNetworkBattle07PriorAircraft[InPacket.UdpObjPlayerNumber] != InPacket.UdpObjVehicle) { //  Get here if this network peer is in a different type of aircraft
                //
