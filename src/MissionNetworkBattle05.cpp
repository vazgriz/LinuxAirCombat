...
void MissionNetworkBattle05::start ()
    ...
    //
    if (! (NetworkMode & 16))
       { // Get here if LAC is NOT configured to ignore espeak
    ...
    if (MyNetworkId % 2)
       ...
       { //@ Get here if player is on BlueTeam
       display ((char *)"MissionNetworkBattle05: Spawning player at -500, 15000", LOG_MOST);
       fplayer->tl->x = -500; //@ was -340
       fplayer->tl->z = 15000.00;
       }
...
int MissionNetworkBattle05::processtimer (Uint32 dt)
    ...
    if (!MissionRunning)
       ...
       */
       ...
       for (i=1; i<=10; i++)
          { // Start all ten bots at medium altitude
    ...
    if (MissionStateNetworkBattle == 2 && (timer > PriorStateEndTimer +1100))
       { //  Only get here once, 1100 ms after MissionStateNetworkBattle
       //  advances to "2":
       //
    ...
    if (MissionStateNetworkBattle == 5 && (timer > PriorStateEndTimer +3300))
       { //  Only get here once, 3300 ms after MissionStateNetworkBattle
       //  advances to "5":
    ...
    if (MissionNetworkBattle05Timer > 1000)
       ...
       //
       if (OnTheGround)
          { //@ If landed ANYWHERE safely, attempt some minor repairs for the distressed pilot
          if (fplayer->realspeed < ((fplayer->StallSpeed) * 0.2))
             { //@ Get here if ground speed is very low. Commence repairs, refuel, etc.
             ...
             { //@ Get here if ground speed is still too high for ground services
             if (!fplayer->UndercarriageLevel)
                { //@ Get here if landing gear is not extended. Accumulate damage
    ...
    if (NetworkReceiveTimer > NetworkReceiveTimerInterval )
       ...
       if (BytesReceived == sizeof (LacUdpApiPacket))
          ...
          if (MissionNetworkBattle05PriorAircraft[InPacket.UdpObjPlayerNumber] != InPacket.UdpObjVehicle)
